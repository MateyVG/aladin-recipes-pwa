// src/components/recipes/categories/duner/DunerSamurai.jsx
...
const DunerSamurai = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      { step: 1, title: t.prepareRedCabbageMix || "Подготовка на смес от червено зеле и червен лук", description: t.prepareRedCabbageMixDesc || "Килограм червено зеле се нарязва на четири части и после на машина с приставка 'Е2' до резени с дебелина 2-3 mm. Половин килограм червен лук се нарязва на ситни полумесеци и се добавя към нарязаното червено зеле, като се прибавя 20g сол и 20g салатена заливка." },
      { step: 2, title: t.grillCabbageMix || "Запичане на зеленчуковата смес", description: t.grillCabbageMixDesc || "Готовата смес се обърква ръчно и се запича с малко олио на скара за около 4-5 минути до готовност. Съхранява се в гастронорма." },
      { step: 3, title: t.grillMeatWithCabbage || "Запичане на месото със смес", description: t.grillMeatWithCabbageDesc || "Пилешкото или телешкото месо от плик се запича леко на скара, като се прибавя от вече готовата смес от зеле и лук." },
      { step: 4, title: t.addFirstSauceLayer || "Добавяне на първи слой сос", description: t.addFirstSauceLayerDesc || "В средата на тортилата се добавят около 50g сос Самурай (лют) или сос Бургер (нелют) - в зависимост от поръчката. Добавят се киселите краставички и месото (пилешко или телешко)." },
      { step: 5, title: t.addSecondSauceLayer || "Добавяне на втори слой сос", description: t.addSecondSauceLayerDesc || "Добавят се още около 50g от съответния сос (Самурай или Бургер) отгоре на пълнежа." },
      { step: 6, title: t.foldTortilla || "Сгъване на тортилата", description: t.foldTortillaDesc || "Дюнерът се загъва по специалния начин - първо се сгъва долната част нагоре, след това двете страни навътре, образувайки плик." },
      { step: 7, title: t.grillDuner || "Запичане на дюнера", description: t.grillDunerDesc || "Тортилата се запича с малко олио на скара за около минута от всяка страна до получаване на приятен златист цвят." },
      { step: 8, title: t.wrapInFoil || "Опаковане", description: t.wrapInFoilDesc || "Готовият дюнер се опакова в каширано алуминиево фолио с лого на компанията." }
    ],
    tips: [
  ...
  return (<><style>{CSS}</style>
    <div style={{ minHeight: '100vh', backgroundColor: DS.color.bg, fontFamily: DS.font, color: DS.color.graphite }}>
      ...
      <div style={{
        ...
        <button onClick={() => setIsFavorite(!isFavorite)} onMouseEnter={() => setHoverFav(true)} onMouseLeave={() => setHoverFav(false)} style={{
          ...
          cursor: 'pointer', fontFamily: DS.font, fontSize: '11px', fontWeight: 600, color: '#fff', transition: 'all 0.15s',
        }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: pad }}>
        ...
        <div style={{ backgroundColor: DS.color.surface, borderRadius: DS.radius, boxShadow: DS.shadow.sm, border: `1px solid ${DS.color.borderLight}`, overflow: 'hidden', marginBottom: '12px', animation: 'cf 0.4s ease' }}>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', top: '10px', right: '10px', backgroundColor: DS.color.surface, padding: '4px 10px', borderRadius: DS.radius, boxShadow: DS.shadow.md, display: 'flex', alignItems: 'center', gap: '4px', border: `1px solid ${DS.color.borderLight}` }}>
              {ic.star('#EAB308')}
              <span style={{ fontFamily: DS.font, fontWeight: 700, fontSize: '13px', color: DS.color.graphite }}>{recipeData.rating}</span>
            </div>
          </div>
          <div style={{ padding: pad }}>
            <p style={{ fontFamily: DS.font, fontSize: '13px', color: DS.color.graphiteMed, lineHeight: 1.6, margin: '0 0 14px' }}>{recipeData.description}</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {[
                ...
                <div key={i} style={{ backgroundColor: DS.color.cardHeader, borderRadius: DS.radius, padding: '10px', textAlign: 'center', border: `1px solid ${DS.color.borderLight}` }}>
        ...
        {recipeData.video && (
          <div style={{ backgroundColor: DS.color.surface, borderRadius: DS.radius, boxShadow: DS.shadow.sm, border: `1px solid ${DS.color.borderLight}`, overflow: 'hidden', marginBottom: '12px', animation: 'cf 0.4s ease' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px', backgroundColor: DS.color.cardHeader, borderBottom: `1px solid ${DS.color.borderLight}` }}>
              ...
              <span style={{ fontFamily: DS.font, fontSize: '12px', fontWeight: 700, color: DS.color.primary, textTransform: 'uppercase', letterSpacing: '0.02em' }}>{t.videoGuide || 'Видео ръководство'}</span>
            </div>
          ...
          <div style={{ display: 'flex', borderBottom: `2px solid ${DS.color.borderLight}` }}>
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
                ...
                cursor: 'pointer', borderBottom: activeTab === tab.id ? `3px solid ${DS.color.primary}` : '3px solid transparent',
          ...
          <div style={{ padding: pad }}>
            {activeTab === 'recipe' && (
              <div>
                ...
                <div style={{ backgroundColor: DS.color.cardHeader, padding: '10px 14px', marginBottom: '1px', borderBottom: `1px solid ${DS.color.borderLight}`, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontFamily: DS.font, fontSize: '12px', fontWeight: 700, color: DS.color.primary, textTransform: 'uppercase', letterSpacing: '0.02em' }}>{t.ingredients || 'Съставки'}</span>
                  <span style={{ fontFamily: DS.font, fontSize: '11px', color: DS.color.graphiteMuted, marginLeft: 'auto' }}>{recipeData.ingredients.length} {t.items || 'артикула'}</span>
                </div>
                {/* Zebra rows */}
                <div style={{ marginBottom: '20px' }}>
                  {recipeData.ingredients.map((ingredient, index) => (
                    <div key={index} style={{
                      ...
                      <span style={{ fontFamily: DS.font, fontSize: '12px', fontWeight: 700, color: DS.color.primary, backgroundColor: DS.color.cardHeader, padding: '2px 8px', borderRadius: DS.radius, whiteSpace: 'nowrap' }}>{ingredient.amount}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ backgroundColor: DS.color.cardHeader, padding: '10px 14px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {ic.dot(DS.color.primary)}
                  <span style={{ fontFamily: DS.font, fontSize: '12px', fontWeight: 700, color: DS.color.primary, textTransform: 'uppercase', letterSpacing: '0.02em' }}>{t.steps || 'Стъпки за приготвяне'}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {recipeData.instructions.map((instruction) => {
                    const isHov = hoverStep === instruction.step;
                    return (
                      <div key={instruction.step} onMouseEnter={() => setHoverStep(instruction.step)} onMouseLeave={() => setHoverStep(null)} style={{
                        backgroundColor: DS.color.surfaceAlt, borderRadius: DS.radius, padding: mob ? '10px' : '14px',
                        borderLeft: `4px solid ${DS.color.primary}`, transition: 'all 0.15s',
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                          <div style={{ flexShrink: 0, width: '28px', height: '28px', borderRadius: '50%', backgroundColor: DS.color.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: DS.font, fontWeight: 700, fontSize: '13px' }}>{instruction.step}</div>
                          <div style={{ flex: 1 }}>
            ...
            {activeTab === 'tips' && (
              <div>
                <div style={{ backgroundColor: DS.color.cardHeader, padding: '10px 14px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {ic.dot(DS.color.primary)}
                  <span style={{ fontFamily: DS.font, fontSize: '12px', fontWeight: 700, color: DS.color.primary, textTransform: 'uppercase', letterSpacing: '0.02em' }}>{t.professionalTips || 'Професионални съвети'}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {recipeData.tips.map((tip, index) => {
                    const isHov = hoverTip === index;
                    return (
                      <div key={index} onMouseEnter={() => setHoverTip(index)} onMouseLeave={() => setHoverTip(null)} style={{
                        backgroundColor: DS.color.cardHeader, borderRadius: DS.radius, padding: '10px 14px',
                        borderLeft: `3px solid ${DS.color.primary}`, display: 'flex', alignItems: 'flex-start', gap: '8px',
                        transition: 'all 0.15s', transform: isHov ? 'translateX(4px)' : 'translateX(0)',
                        animation: `stagger 0.3s ease ${index * 40}ms both`,
                      }}>
                        <div style={{ flexShrink: 0, width: '22px', height: '22px', borderRadius: '50%', backgroundColor: DS.color.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: DS.font, fontWeight: 700, fontSize: '11px' }}>{index + 1}</div>
