...
/* ═══════════════════════════════════════════════════════════════
   ...
   1. SIZE 260→220, OUTER_R 105→85, INNER_R 70→55 (събира се на екрана)
   2. Модал центриран (alignItems:'center') вместо bottom sheet
   3. handlePointerUp — след минути: авто-затваряне + запис
   ═══════════════════════════════════════════════════════════════ */
const ClockFacePicker = ({ value, onChange, label: cfLabel, style: cfStyle }) => {
  ...
  const SIZE = 220, CENTER = SIZE / 2, OUTER_R = 85, INNER_R = 55;
  ...
  const handlePointerUp = () => {
    if (dragging) {
      ...
      if (mode === 'hour') {
        ...
        // Авто-затваряне след избор на минути
        setTimeout(() => {
          setOpen(false);
          if (selHour !== null) {
            const h = String(selHour).padStart(2, '0');
  ...
  return (
    <div style={cfStyle}>
      ...
      <div onClick={openPicker} style={{
        ...
        borderRadius: DS.radius, transition: 'all 150ms', minWidth: '100px', width: '100%',
      }}>
        <span style={{ fontSize: '16px', fontWeight: 700, color: hasVal ? DS.color.graphite : DS.color.graphiteMuted, letterSpacing: '0.05em', flex: 1 }}>
      ...
      {open && (
        <div onClick={() => setOpen(false)} style={{
          position: 'fixed', inset: 0, backgroundColor: 'rgba(30,42,38,0.55)', backdropFilter: 'blur(4px)',
          display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '16px',
        }}>
          <div onClick={e => e.stopPropagation()} style={{
            width: '100%', maxWidth: '300px', backgroundColor: DS.color.surface,
            borderRadius: '12px', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.25)',
          }}>
            <div style={{ backgroundColor: DS.color.primary, padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
              <span onClick={() => setMode('hour')} style={{ fontSize: '36px', fontWeight: 700, fontFamily: DS.font, color: mode === 'hour' ? 'white' : 'rgba(255,255,255,0.4)', cursor: 'pointer', lineHeight: 1 }}>
                {selHour !== null ? String(selHour).padStart(2, '0') : '--'}
              </span>
              <span style={{ fontSize: '36px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', lineHeight: 1 }}>:</span>
              <span onClick={() => setMode('minute')} style={{ fontSize: '36px', fontWeight: 700, fontFamily: DS.font, color: mode === 'minute' ? 'white' : 'rgba(255,255,255,0.4)', cursor: 'pointer', lineHeight: 1 }}>
                {selMin !== null ? String(selMin).padStart(2, '0') : '00'}
              </span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', paddingTop: '10px' }}>
              {['hour', 'minute'].map(m => (
                <div key={m} onClick={() => setMode(m)} style={{ width: 8, height: 8, borderRadius: '50%', cursor: 'pointer', backgroundColor: mode === m ? DS.color.primary : DS.color.borderLight, transition: 'all 150ms' }} />
              ))}
            </div>
            <div style={{ padding: '10px 16px 8px', display: 'flex', justifyContent: 'center' }}>
              <div ref={clockRef}
                ...
                style={{ width: SIZE, height: SIZE, borderRadius: '50%', backgroundColor: '#EFF2F0', position: 'relative', cursor: 'pointer', touchAction: 'none', userSelect: 'none', flexShrink: 0 }}
              >
                ...
                {showHand && (
                  <svg style={{ position: 'absolute', inset: 0, zIndex: 2, pointerEvents: 'none' }} width={SIZE} height={SIZE}>
                    <line x1={CENTER} y1={CENTER} x2={handEndX} y2={handEndY} stroke={DS.color.primary} strokeWidth="2" strokeLinecap="round" />
                    <circle cx={handEndX} cy={handEndY} r="16" fill={DS.color.primary} opacity="0.18" />
                  </svg>
                )}
                {mode === 'hour' ? (<>
                  {outerHours.map((h, i) => { const p = getPos(i, OUTER_R); const s = selHour === h; return (
                    <div key={`o${h}`} style={{ position: 'absolute', left: p.x - 16, top: p.y - 16, width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: s ? 700 : 500, fontFamily: DS.font, color: s ? 'white' : DS.color.graphite, backgroundColor: s ? DS.color.primary : 'transparent', transition: 'all 120ms', zIndex: 4 }}>{h}</div>
                  ); })}
                  {innerHours.map((h, i) => { const p = getPos(i, INNER_R); const s = selHour === h; return (
                    <div key={`i${h}`} style={{ position: 'absolute', left: p.x - 14, top: p.y - 14, width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: s ? 700 : 500, fontFamily: DS.font, color: s ? 'white' : DS.color.graphiteMuted, backgroundColor: s ? DS.color.primary : 'transparent', transition: 'all 120ms', zIndex: 4 }}>{String(h).padStart(2, '00')}</div>
                  ); })}
                </>) : (<>
                  {minuteMarks.map((m, i) => { const p = getPos(i, OUTER_R); const s = selMin === m; return (
                    <div key={`m${m}`} style={{ position: 'absolute', left: p.x - 16, top: p.y - 16, width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: s ? 700 : 500, fontFamily: DS.font, color: s ? 'white' : DS.color.graphite, backgroundColor: s ? DS.color.primary : 'transparent', transition: 'all 120ms', zIndex: 4 }}>{String(m).padStart(2, '0')}</div>
                  ); })}
                </>)}
              </div>
            </div>
            <div style={{ padding: '8px 20px 16px', display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button onClick={confirm} style={{ padding: '10px 24px', border: 'none', borderRadius: '6px', backgroundColor: DS.color.primary, color: 'white', fontFamily: DS.font, fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>Потвърди</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    ...
    >
      <div style={{
        ...
        <span style={{ fontFamily: DS.font, fontSize: isMobile ? '13px' : '14px', fontWeight: 700, color: DS.color.primary, textTransform: 'uppercase' }}>
          № {production.number}
        </span>
        {!isRice && (
          <div style={{ display: 'flex', gap: '12px' }}>
            <span style={{ fontFamily: DS.font, fontSize: '11px', fontWeight: 600, color: DS.color.primary, backgroundColor: 'rgba(27,94,55,0.08)', padding: '3px 8px', borderRadius: DS.radius }}>{production.fryDuration}</span>
            <span style={{ fontFamily: DS.font, fontSize: '11px', fontWeight: 600, color: DS.color.primary, backgroundColor: 'rgba(27,94,55,0.08)', padding: '3px 8px', borderRadius: DS.radius }}>{production.fryTemperature}</span>
          </div>
        )}
        {canRemove && (
          <button onClick={() => onRemove(type, production.id)} title="Премахни запис"
      ...
      <div style={{ padding: isMobile ? '12px 14px' : '16px 18px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '12px' }}>
        ...
        />
        <ControlInput label="Партида, L" value={production.batchL} onChange={e => onUpdate(type, production.id, 'batchL', e.target.value)} placeholder="Партида" />
        <Time24Select label="Час на приготвяне" value={production.cookingTime} onChange={val => onUpdate(type, production.id, 'cookingTime', val)} />
        <Time24Select label="Час на срок на витрината" value={production.displayTime} onChange={val => onUpdate(type, production.id, 'displayTime', val)} />
        <ControlInput label="Брак" value={production.defect} onChange={e => onUpdate(type, production.id, 'defect', e.target.value)} placeholder="Брак" />
        <div>
          <label style={{ display: 'block', fontFamily: DS.font, fontSize: '11px', fontWeight: 600, color: DS.color.graphiteLight, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '4px' }}>Служител</label>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button onClick={() => onUpdate(type, production.id, 'checked', !production.checked)}
              style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '6px', color: production.checked ? DS.color.primary : '#9CA3AF', transition: 'color 150ms ease', display: 'flex', flexShrink: 0, minWidth: '32px', minHeight: '32px', alignItems: 'center', justifyContent: 'center' }}>
              {production.checked ? <CheckSquare style={{ width: 22, height: 22 }} /> : <Square style={{ width: 22, height: 22 }} />}
            </button>
            <input type="text" value={production.employeeName} onChange={e => onUpdate(type, production.id, 'employeeName', e.target.value)}
              list={`employee-names-${type}-${production.id}`} placeholder="Служител"
              style={{ flex: 1, padding: '10px 12px', minWidth: 0, backgroundColor: DS.color.surfaceAlt, border: `1.5px solid ${DS.color.borderLight}`, borderRadius: DS.radius, fontSize: '14px', fontFamily: DS.font, color: DS.color.graphite, outline: 'none', transition: 'all 150ms ease', boxSizing: 'border-box', WebkitAppearance: 'none' }}
              onFocus={e => { e.target.style.borderColor = DS.color.primary; e.target.style.backgroundColor = DS.color.surface; e.target.style.boxShadow = `0 0 0 3px ${DS.color.primaryGlow}`; }}
              onBlur={e => { e.target.style.borderColor = DS.color.borderLight; e.target.style.backgroundColor = DS.color.surfaceAlt; e.target.style.boxShadow = 'none'; }}
            />
  ...
  <div style={{ marginBottom: isMobile ? '20px' : '28px' }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <h2 style={{ margin: 0, fontFamily: DS.font, fontSize: isMobile ? '16px' : '18px', fontWeight: 700, color: DS.color.primary, textTransform: 'uppercase' }}>{title}</h2>
        <span style={{ fontFamily: DS.font, fontSize: '11px', fontWeight: 600, color: DS.color.graphiteMuted, backgroundColor: DS.color.pendingBg, padding: '2px 8px', borderRadius: DS.radius }}>{productionsList.length}</span>
      </div>
      <button onClick={() => onAdd(type)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '8px 14px', backgroundColor: DS.color.primary, border: 'none', borderRadius: DS.radius, color: 'white', cursor: 'pointer', fontFamily: DS.font, fontSize: '12px', fontWeight: 600, boxShadow: DS.shadow.glow, minHeight: '36px' }}>
        <Plus style={{ width: 14, height: 14 }} />Добави
      </button>
    </div>
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : (isLandscape && isTablet) ? 'repeat(2, 1fr)' : isTablet ? '1fr' : 'repeat(auto-fill, minmax(400px, 1fr))', gap: isMobile ? '10px' : '14px' }}>
      {productionsList.map(prod => (
        <ProductRowCard key={prod.id} type={type} production={prod} onUpdate={onUpdate} onRemove={onRemove} canRemove={productionsList.length > 1} savedEmployees={savedEmployees} isMobile={isMobile} isLandscape={isLandscape} />
      ))}
    </div>
  </div>
);
...
const makeInitialProductions = (baseId) => ({
  file: [{ id: baseId, number: 1, count: '', batchL: '', fryDuration: '4,5 мин', fryTemperature: '171°C', cookingTime: '', displayTime: '', defect: '', employeeName: '', checked: false }],
  bonFile: [{ id: baseId + 1, number: 1, quantity: '', batchL: '', fryDuration: '2,5 мин', fryTemperature: '171°C', cookingTime: '', displayTime: '', defect: '', employeeName: '', checked: false }],
  wings: [{ id: baseId + 2, number: 1, quantity: '', batchL: '', fryDuration: '11,5 мин', fryTemperature: '174°C', cookingTime: '', displayTime: '', defect: '', employeeName: '', checked: false }],
  rice: [{ id: baseId + 3, number: 1, quantity: '', batchL: '', cookingTime: '', displayTime: '', defect: '', employeeName: '', checked: false }],
  butchetta: [{ id: baseId + 4, number: 1, quantity: '', batchL: '', fryDuration: '11,5 мин', fryTemperature: '174°C', cookingTime: '', displayTime: '', defect: '', employeeName: '', checked: false }],
});
...
const ChickenProductionSheet = ({ template, config, department, restaurantId, onBack }) => {
  ...
  const syncPending = async () => {
    ...
    for (const item of queue) {
      try { const { savedAt, ...data } = item; const { error } = await supabase.from('checklist_submissions').insert(data); if (error) throw error; }
      catch { failed.push(item); }
    }
    savePending(failed);
    if (!failed.length) { setSyncStatus('synced'); setAutoSaveStatus(`✓ ${queue.length} ${queue.length === 1 ? 'запис синхронизиран' : 'записа синхронизирани'}`); }
  ...
  const hasAnyData = () => {
    ...
    return Object.values(productions).some(typeArray =>
      typeArray.some(prod => prod.count || prod.quantity || prod.batchL || prod.cookingTime || prod.displayTime || prod.defect || prod.employeeName)
    );
  };
  ...
  useEffect(() => { const interval = setInterval(() => saveDraft(), 30000); return () => clearInterval(interval); }, [productions, manager, savedEmployees]);

  const saveDraft = () => {
    if (!hasAnyData()) return;
    localStorage.setItem(`draft_${template.id}_${currentDate}`, JSON.stringify({ productions, manager, savedEmployees, timestamp: Date.now() }));
    setHasDraft(true);
  ...
  const confirmExit = (saveAndExit) => {
    if (saveAndExit) { saveDraft(); setAutoSaveStatus('Данните са запазени.'); setTimeout(() => onBack(), 1500); }
    else { onBack(); }
  ...
  const addProduction = (type) => {
    ...
    const newProduction = {
      id: Date.now(), number: currentLength + 1,
  ...
  const updateProduction = (type, id, field, value) => {
    setProductions(prev => ({ ...prev, [type]: prev[type].map(p => p.id === id ? { ...p, [field]: value } : p) }));
  ...
  const handleSubmit = async () => {
    ...
    try { const { data: userData } = await supabase.auth.getUser(); if (userData?.user?.id) submissionData.submitted_by = userData.user.id; }
    ...
    if (!navigator.onLine) {
      ...
      setProductions(makeInitialProductions(Date.now())); setManager(''); setLoading(false); return;
    }
    try {
      ...
      setTimeout(() => setAutoSaveStatus(''), 3000);
      setProductions(makeInitialProductions(Date.now())); setManager('');
      ...
      <div style={{ minHeight: '100vh', backgroundColor: DS.color.bg, fontFamily: DS.font, color: DS.color.graphite, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ backgroundColor: DS.color.surface, borderRadius: DS.radius, padding: isMobile ? '24px 20px' : '32px', maxWidth: '480px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'flex-end' }}>
                {[
                  ...
                  <button key={i} onClick={btn.action} style={{ padding: '12px 16px', backgroundColor: btn.bg, color: btn.color, border: 'none', borderRadius: DS.radius, cursor: 'pointer', fontSize: '13px', fontWeight: 600, fontFamily: DS.font, width: isMobile ? '100%' : 'auto' }}>{btn.label}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <div style={{ backgroundColor: DS.color.graphite, padding: isMobile ? '8px 12px' : '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 12px rgba(0,0,0,0.15)', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '6px' : '16px' }}>
            ...
            {autoSaveStatus && (
              <span style={{ fontFamily: DS.font, fontSize: '11px', color: syncStatus === 'error' ? 'rgba(255,200,200,0.9)' : DS.color.ok, display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 500 }}>
                <CheckCircle style={{ width: 12, height: 12 }} />{isMobile ? '✓' : autoSaveStatus}
              </span>
            )}
            {hasDraft && !isMobile && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: 'rgba(255,255,255,0.08)', padding: '4px 10px', borderRadius: DS.radius }}>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: isMobile ? '12px' : '24px', flex: 1, width: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', marginBottom: isMobile ? '16px' : '24px', gap: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '10px' : '14px' }}>
              ...
              <div style={{ minWidth: 0 }}>
                <h1 style={{ fontSize: isMobile ? '16px' : '22px', fontWeight: 700, color: DS.color.primary, margin: 0, textTransform: 'uppercase', fontFamily: DS.font }}>ПРОИЗВОДСТВЕН ЛИСТ ЧИКЪН</h1>
              </div>
            </div>
          </div>

          <div style={{ backgroundColor: DS.color.surface, borderRadius: DS.radius, padding: pad, marginBottom: isMobile ? '16px' : '24px', boxShadow: DS.shadow.sm, border: `1px solid ${DS.color.borderLight}`, display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'flex-end', gap: isMobile ? '12px' : '20px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: isMobile ? 1 : 'none' }}>
              <div style={{ padding: '6px', borderRadius: DS.radius, backgroundColor: DS.color.okBg, flexShrink: 0 }}><Calendar style={{ width: 16, height: 16, color: DS.color.primary }} /></div>
              <ControlInput label="Дата" type="date" value={currentDate} onChange={e => setCurrentDate(e.target.value)} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1, minWidth: 0 }}>
              <div style={{ padding: '6px', borderRadius: DS.radius, backgroundColor: DS.color.okBg, flexShrink: 0 }}><Building2 style={{ width: 16, height: 16, color: DS.color.primary }} /></div>
              <div style={{ flex: 1, minWidth: 0 }}><ControlInput label="Управител" value={manager} onChange={e => setManager(e.target.value)} placeholder="Име и фамилия" /></div>
            </div>
            <div style={{ display: 'flex', gap: '8px', marginLeft: isMobile ? 0 : 'auto' }}>
              {hasDraft && (
                <button onClick={clearDraft} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px 14px', backgroundColor: 'transparent', border: `1.5px solid ${DS.color.danger}`, borderRadius: DS.radius, color: DS.color.danger, cursor: 'pointer', fontFamily: DS.font, fontSize: '12px', fontWeight: 600, minHeight: '40px' }}>
                  <RotateCcw style={{ width: 14, height: 14 }} />{isMobile ? 'Нов лист' : 'Нов производствен лист'}
                </button>
              )}
            </div>
          </div>

          <ProductSection type="file" title="Филе" productionsList={productions.file} onAdd={addProduction} onRemove={removeProduction} onUpdate={updateProduction} savedEmployees={savedEmployees} isMobile={isMobile} isTablet={isTablet} isLandscape={isLandscape} />
          <ProductSection type="bonFile" title="Бон филе" productionsList={productions.bonFile} onAdd={addProduction} onRemove={removeProduction} onUpdate={updateProduction} savedEmployees={savedEmployees} isMobile={isMobile} isTablet={isTablet} isLandscape={isLandscape} />
          <ProductSection type="wings" title="Крилца" productionsList={productions.wings} onAdd={addProduction} onRemove={removeProduction} onUpdate={updateProduction} savedEmployees={savedEmployees} isMobile={isMobile} isTablet={isTablet} isLandscape={isLandscape} />
          <ProductSection type="rice" title="Ориз" productionsList={productions.rice} onAdd={addProduction} onRemove={removeProduction} onUpdate={updateProduction} savedEmployees={savedEmployees} isMobile={isMobile} isTablet={isTablet} isLandscape={isLandscape} />
          <ProductSection type="butchetta" title="Бутчета" productionsList={productions.butchetta} onAdd={addProduction} onRemove={removeProduction} onUpdate={updateProduction} savedEmployees={savedEmployees} isMobile={isMobile} isTablet={isTablet} isLandscape={isLandscape} />
          ...
          <div style={{ backgroundColor: DS.color.surface, borderRadius: DS.radius, padding: pad, boxShadow: DS.shadow.md, border: `1px solid ${DS.color.borderLight}`, textAlign: 'center' }}>
            <button onClick={handleSubmit} disabled={loading} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: isMobile ? '14px 20px' : '16px 48px', width: isMobile ? '100%' : 'auto', backgroundColor: loading ? DS.color.graphiteMuted : DS.color.primary, border: 'none', borderRadius: DS.radius, color: 'white', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: DS.font, fontSize: isMobile ? '14px' : '16px', fontWeight: 600, boxShadow: loading ? 'none' : '0 4px 12px rgba(25,94,51,0.3)', animation: !loading && hasAnyData() ? 'ctrlBreathe 3s ease-in-out infinite' : 'none', minHeight: '48px' }}>
              <Save style={{ width: 20, height: 20 }} />
              {loading ? 'Запазване...' : (isMobile ? 'Запази и започни нов' : 'Запази производствен лист и започни нов')}
            </button>
            <p style={{ marginTop: '10px', fontSize: '13px', color: DS.color.graphiteLight, fontFamily: DS.font }}>След запазване формата се изчиства за нов лист</p>
          </div>
        </div>
