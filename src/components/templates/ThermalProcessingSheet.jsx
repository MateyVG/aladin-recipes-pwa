// src/components/checklists/ThermalProcessingSheet.jsx
...
/* ═══════════════════════════════════════════════════════════════
   MAIN COMPONENT
   ═══════════════════════════════════════════════════════════════ */
const ThermalProcessingSheet = ({ onBack, user, restaurantId, restaurantName }) => {
  const mob = useR();
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        checklist_type: 'thermal_processing',
        form_code: 'НСL 02',
        data: {
  ...
  const generatePrintHTML = () => {
    ...
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>ПРОИЗВОДСТВЕН ЛИСТ – ТЕРМИЧНА ОБРАБОТКА</title>
    <style>
      body{font-family:'DM Sans',Arial,sans-serif;padding:20px;color:#1E2A26}
      h1{color:#1B5E37;font-size:15px;text-align:center;margin:10px 0}
      ...
      .legend{margin-top:12px;font-size:11px;color:#3D4F48}
      .legend span{display:inline-block;width:24px;height:24px;line-height:24px;text-align:center;font-weight:700;margin-right:8px;border:1px solid #D5DDD9}
    </style></head><body>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
      <img src="${LOGO}" style="height:40px" onerror="this.style.display='none'"/>
      <div>
        <h1 style="text-align:left;margin:0">ПРОИЗВОДСТВЕН ЛИСТ – ТЕРМИЧНА ОБРАБОТКА<br/>СТЕПЕН НА ИЗПЪРЖВАНЕ</h1>
        <div style="font-size:10px;color:#6B7D76">Код: НСL 02 • Редакция: 00</div>
      </div>
    </div>
    <div class="meta">
    ...
    <table>
      <thead><tr>
        <th>Дата и час на изпържване</th><th>Количество</th><th>Партида, L</th>
        <th>Степен на изпържване</th><th>Забележки</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
    <div class="legend">
      <strong>Легенда:</strong><br/>
      <span style="background:#E8F5EE;color:#1B8A50">A</span> Леко &nbsp;
      <span style="background:#EFF6FF;color:#2563EB">B</span> Средно &nbsp;
      <span style="background:#FFFBEB;color:#C47F17">C</span> Силно &nbsp;
      <span style="background:#FFF5F5;color:#C53030">D</span> Препържено
    </div>
    ...
    <div style={{ minHeight: '100vh', backgroundColor: DS.color.bg, fontFamily: DS.font, color: DS.color.graphite }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: pad }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: mob ? '10px' : '14px', marginBottom: '16px', animation: 'cf 0.4s ease' }}>
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{ fontFamily: DS.font, fontSize: mob ? '13px' : '19px', fontWeight: 700, color: DS.color.primary, margin: 0, letterSpacing: '-0.01em', lineHeight: 1.25, textTransform: 'uppercase' }}>
              Производствен лист – Термична обработка
            </h1>
            <p style={{ fontFamily: DS.font, fontSize: mob ? '10px' : '12px', color: DS.color.graphiteLight, fontWeight: 500, margin: '3px 0 0' }}>
              Степен на изпържване • Код: НСL 02 • Редакция: 00 {restaurantName && `• ${restaurantName}`}
            </p>
          </div>
        </div>
        ...
        <div style={{
          ...
          border: `1px solid ${DS.color.borderLight}`, overflow: 'hidden', marginBottom: '12px', animation: 'cf 0.4s ease',
        }}>
          ...
          <div style={{ padding: '10px 14px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {DEGREES.map(d => (
              <div key={d.value} style={{
                flex: 1, minWidth: mob ? '70px' : '120px', textAlign: 'center', padding: '8px',
                backgroundColor: d.bg, border: `1px solid ${d.color}30`, borderRadius: DS.radius,
              }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap', animation: 'cf 0.4s ease' }}>
          <button onClick={addRecord} style={{
            ...
            borderRadius: DS.radius, fontFamily: DS.font, fontSize: '13px', fontWeight: 600, cursor: 'pointer',
          }}>
          ...
          return (
            <div key={record.id}
              ...
              style={{
                ...
                overflow: 'hidden', marginBottom: '10px', transition: 'all 0.15s',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {record.degree && (
                    <span style={{
                      ...
                      padding: '2px 10px', borderRadius: DS.radius, ...degStyle,
                    }}>
                      Степен {record.degree}
                    </span>
                  )}
                </div>
                {records.length > 1 && (
              ...
              <div style={{ padding: pad }}>
                ...
                {/* Row 3: Degree selection (A/B/C/D) */}
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontFamily: DS.font, fontSize: '11px', fontWeight: 600, color: DS.color.graphiteLight, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '6px' }}>
                    Степен на изпържване
                  </label>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    {DEGREES.map(deg => {
                      const selected = record.degree === deg.value;
                      return (
                        <button key={deg.value} onClick={() => updateRecord(idx, 'degree', deg.value)} style={{
                          flex: 1, padding: mob ? '12px 8px' : '14px 12px',
                          backgroundColor: selected ? deg.bg : DS.color.surfaceAlt,
                          border: `2px solid ${selected ? deg.color : DS.color.borderLight}`,
                          borderRadius: DS.radius, cursor: 'pointer',
                          fontFamily: DS.font, fontWeight: 700,
                          color: selected ? deg.color : DS.color.graphiteMuted,
                          transition: 'all 0.15s',
                          boxShadow: selected ? `0 0 0 3px ${deg.color}15` : 'none',
                          textAlign: 'center',
                        }}>
          ...
          <div style={{
            ...
            <div style={{ padding: pad, display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {DEGREES.map(deg => {
                ...
                return (
                  <div key={deg.value} style={{
                    flex: 1, minWidth: '70px', textAlign: 'center', padding: '12px',
                    backgroundColor: count > 0 ? deg.bg : DS.color.surfaceAlt,
                    border: `1px solid ${count > 0 ? deg.color + '30' : DS.color.borderLight}`,
                    borderRadius: DS.radius, opacity: count > 0 ? 1 : 0.5,
                  }}>
